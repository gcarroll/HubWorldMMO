...
| `URiftMutableEquipmentComponent` | UActorComponent | Finds body mesh by Rift.Component.BodyMesh ComponentTag. Applies Mutable parameters on equip/unequip. |
...
Player input
  ...
  → OnItemEquipped broadcast (server + client via FastArray PostReplicatedAdd)
      → URiftMutableEquipmentComponent::OnItemEquipped
          → ApplyMutableParameters(ActiveMutableParameters) → UCustomizableObjectInstance::UpdateSkeletalMeshAsync
```
...
Player input → Send gameplay event Tag_Rift_Ability_Equip [Server]
    ...
    → OnItemEquipped.Broadcast(Item, SlotTag) [server + client via replication]
        → URiftMutableEquipmentComponent → ApplyMutableParameters → Mutable async regen
...
APawn destroyed (URiftInventoryComponent on PlayerState survives)
    ...
    → OnInventoryReady: binds OnInventoryItemEvent to detect forced item removal
...
**Properties:** `EquipmentSlotTag`, `GrantedAbilities`, `WeaponSocketName`, `ActiveMutableParameters`, `HolsteredMutableParameters`
...
*End of RiftVault Technical Design Document v1.0.4*
